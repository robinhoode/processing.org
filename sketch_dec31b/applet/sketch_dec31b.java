...
public class sketch_dec31b extends PApplet {
...
static class Matrix3D {
  ...
  public PVector mult(PVector vector) {
...
class Cube {
  ...
  public void drawVertex(PVector vector) {
  ...
  public void moveTowards(PVector init, PVector nudgeVector) {
    this.location = init;
    nudge(nudgeVector);
  }
    
  public void nudge(PVector offset) {
...
class CubeArray {
  ...
  public void constantNudge(float x, float y, float z) {
    for (int i = 0; i < cubes.length; i++) {
  ...
  public void relativeNudge(float x, float y, float z) {
    for (int i = 0; i < cubes.length; i++) {
  ...
  public void constantRotateX(float angle) {
    for (int i = 0; i < cubes.length; i++) {
      cubes[i].rotateX(angle);
    }
  }
  
  public void constantRotateY(float angle) {
    for (int i = 0; i < cubes.length; i++) {
      cubes[i].rotateY(angle);
    }
  }
  
  public void constantRotateZ(float angle) {
    for (int i = 0; i < cubes.length; i++) {
      cubes[i].rotateZ(angle);
    }
  }
  
  public void relativeRotateXThenNudgeXY(float angle, float x) {
  ...
  public void moveRelativeToPrev(PVector nudge) {
  ...
  public void nudgeArray(PVector nudge) {
  ...
  public void nudgeRotateArrayX(float angle) {
    this.angleFacing.x += angle;
    for (int i = 1; i < cubes.length; i++) {
  ...
  public void nudgeRotateArrayY(float angle) {
    this.angleFacing.y += angle;
    for (int i = 1; i < cubes.length; i++) {
...
CubeArray cubes = new CubeArray(new PVector(20, 20, 20), new PVector(25, 25, 25), new PVector(0,0,0), 10);
int y = 0;
...
public void keyPressed() {
  if (keyCode == UP) 
    cubes.nudgeArray(new PVector( 0, -1, 0));
    
  if (keyCode == DOWN)
    cubes.nudgeArray(new PVector( 0,  1, 0));
    
  if (keyCode == RIGHT)
    cubes.nudgeArray(new PVector( 1,  0, 0));
    
  if (keyCode == LEFT)
    cubes.nudgeArray(new PVector(-1,  0, 0));
    
  if (key == 'q')
    cubes.nudgeRotateArrayY(PI/32);
}



  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#E2E2E2", "sketch_dec31b" });
  }
}
