...
class ThreeDIcosahedron {
  ...
  public void plotVertex(PVector vector) {
    vertex(vector.x, vector.y, vector.z);
  }
  
  public void drawLinesFor(int i) {
...
//    drawLinesFor(7);
  ...
  public void drawLine(PVector myPoint, PVector otherPoint) {
    stroke(0, 0, 255, 255);
    line(myPoint.x, myPoint.y, myPoint.z, otherPoint.x, otherPoint.y, otherPoint.z);    
  }
  
  public PVector flipSignOf(PVector vector, float value) {
  ...
  public PVector rotateLeft(PVector vector) {
  ...
  public PVector rotateRight(PVector vector) {
    if (signCount(vector) == 1)
  ...
  public int signCount(PVector vector) {
    int count = 0;
    if (vector.x < 0)
      count += 1;
    if (vector.y < 0)
...
//  icosa.fillRectangles();
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#D4D0C8", "Icosahedron" });
  }
}
