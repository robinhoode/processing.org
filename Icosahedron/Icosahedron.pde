...
class ThreeDIcosahedron {
  ...
  PVector flipSignOf(PVector vector, float value) {
    if (vector.x == value || vector.x == -value)
      ...
      throw new RuntimeException("Cannot find +/- value pair!");
  }  
...
void draw() {
  ...
  ThreeDIcosahedron icosa = new ThreeDIcosahedron(10);
  icosa.drawLines();
